$(function () {
  ...
  $('#cuti-pdf_file').on('change', function (evt) {
  ...
  $('#btn-upload').on('click', function () {
    var hasError = false;
    console.log($('#cuti-pdf_file').prop('files')[0], $('#cuti-pdf_file').prop('files')[0]);
    if (!$('#cuti-pdf_file').prop('files')[0]) {
      hasError = true;
      alert('PDF belum dipilih');
    }
    if ($('#cuti-status').val() == 5) {
      hasError = true;
